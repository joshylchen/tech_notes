# Table of contents

* [Tech Notes](README.md)

## ⚛ SAS\_Pyspark

* [SAS\_Merge and Pyspark](sas\_pyspark/sas\_merge-and-pyspark.md)
* [Replicate SAS\_merge using pyspark](sas\_pyspark/replicate-sas\_merge-using-pyspark.md)
* [Efficient replication of SAS counter function using Pyspark](sas\_pyspark/efficient-replication-of-sas-counter-function-using-pyspark.md)

***

* [Pyspark Case Senstivity](pyspark-case-senstivity.md)
* [Performing DateTime operation on multiple columns using Pyspark](performing-datetime-operation-on-multiple-columns-using-pyspark.md)

## 🧠 Machine Learning and Data Analytics

* [The intuition behind the Linear Discrimination Model - DevOps edition](machine-learning-and-data-analytics/the-intuition-behind-the-linear-discrimination-model-devops-edition.md)

## ☁ Cloud and Data Architecture

* [Parameterizing dynamic variables in Azure Synapse Pipeline](cloud-and-data-architecture/parameterizing-dynamic-variables-in-azure-synapse-pipeline.md)
